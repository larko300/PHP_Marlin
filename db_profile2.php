...
$user_image['name'] = iuniqid().substr($_FILES['image'], -4);
...
if (!empty($new_name_user)&&!empty($new_user_email)&&$email_check==true&&empty($recurrent_mail)&&$new_name_user!==$_SESSION['name_varify']&&$new_user_email!==$_SESSION['email_varify']) {
...
if (empty($new_name_user)) {
  $_SESSION['flash_profile_name'] = 'Введите имя';
  header ("location:http://localhost/Marlin_Materialy/profile.php");
}
if (empty($new_user_email)) {
  $_SESSION['flash_profile_email'] = 'Введите имейл';
  header ("location:http://localhost/Marlin_Materialy/profile.php");
}
